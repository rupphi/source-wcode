...
export function PrintHistoryView({ shopId }: { shopId: number }) {
  ...
  return (
    <div className="grid gap-5">
      <section className="grid gap-3 sm:grid-cols-3" aria-label="Сводка истории печати">
        <HistoryMetric
          icon={Clock3}
          label="Всего заданий"
      ...
      <section className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-4 shadow-[var(--shadow-panel)] md:p-5">
        <form className="flex flex-col gap-3 sm:flex-row" onSubmit={submitSearch} role="search">
          <label className="relative min-w-0 flex-1">
            <span className="sr-only">Поиск истории печати</span>
            ...
            <input
              ...
              placeholder="Поставка, ID задания или шаблон"
              aria-label="Поиск истории печати"
            />
          </label>
        ...
        <div className="mt-4 flex flex-wrap gap-2" aria-label="Фильтр истории печати">
          <FilterButton active={status === "all"} onClick={() => selectStatus("all")}>Все <Count value={successfulItems + failedItems} loading={visibleState.status === "loading"} /></FilterButton>
          <FilterButton active={status === "success"} onClick={() => selectStatus("success")}>Успешные <Count value={successfulItems} loading={visibleState.status === "loading"} /></FilterButton>
          <FilterButton active={status === "failed"} onClick={() => selectStatus("failed")}>Ошибки <Count value={failedItems} loading={visibleState.status === "loading"} /></FilterButton>
        </div>
      </section>

      {reprintState.status === "success" && (
        <ReprintResult state={reprintState} onOpen={openReprint} />
      )}
      {reprintState.status === "error" && (
        <div className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900" role="alert">
          ...
          <p><strong>Повторная печать не завершена.</strong> Файлы истории не изменены. Попробуйте снова.</p>
        </div>
      )}
      ...
      {visibleState.status === "loading" ? (
        ...
        <HistoryError onRetry={() => setRetryKey((value) => value + 1)} />
      ) : visibleState.data.items.length === 0 ? (
        <HistoryEmpty filtered={query.length > 0 || status !== "all"} />
      ) : (
        <HistoryTable
      ...
      {visibleState.status === "ready" && visibleState.data.items.length > 0 && (
        <Pagination
          ...
          onPage={setPage}
          ariaLabel="Пагинация истории печати"
          previousLabel="Предыдущая страница истории"
          nextLabel="Следующая страница истории"
        />
      )}
    </div>
  );
...
function HistoryMetric({ icon: Icon, label, value, tone, loading }: {
  icon: typeof Clock3;
  label: string;
  value: number;
...
function HistoryTable({ items, runningJobId, onReprint }: {
        ...
        <table className="w-full min-w-[54rem] border-collapse text-left">
          <thead className="border-b border-[var(--border-subtle)] bg-[var(--surface-muted)]/70">
            <tr className="text-xs font-semibold tracking-[0.04em] text-[var(--text-secondary)] uppercase">
              <th className="px-5 py-3.5" scope="col">Дата печати</th>
              <th className="px-4 py-3.5" scope="col">Поставка</th>
              <th className="px-4 py-3.5" scope="col">Шаблон</th>
              <th className="px-4 py-3.5 text-right" scope="col">Этикеток</th>
              <th className="px-5 py-3.5" scope="col">Статус</th>
            ...
            {items.map((item) => (
              <tr className="transition hover:bg-[var(--surface-muted)]/55" key={item.jobId}>
                <td className="px-5 py-4 text-sm text-[var(--text-secondary)]">
                  <span className="inline-flex items-center gap-2 whitespace-nowrap"><CalendarDays aria-hidden="true" size={15} />{formatPrintedAt(item.printedAt)}</span>
                  <p className="mt-1 font-mono text-[0.68rem] text-[var(--text-muted)]">Job #{item.jobId}</p>
                </td>
                ...
                <td className="px-4 py-4 text-sm text-[var(--text-secondary)]">
                  <span className="inline-flex items-center gap-2"><FileText aria-hidden="true" size={15} />{item.templateName || "По умолчанию"}</span>
                </td>
                ...
                <td className="px-5 py-4 text-right">
                  {item.canReprint ? (
                    ...
                    >
                      ...
                      {runningJobId === item.jobId ? "Создаём PDF" : "Повторить"}
                    </button>
                  ) : <span className="text-sm text-[var(--text-muted)]" aria-label="Повторная печать недоступна">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
...
function ReprintResult({ state, onOpen }: {
    ...
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950 shadow-[var(--shadow-panel)]" aria-live="polite">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="font-semibold">PDF созданы повторно</h3>
          <p className="mt-1 text-sm text-emerald-800">{numberFormat.format(state.data.itemCount)} этикеток сохранено. Открытие доступно только для файлов этой операции.</p>
          <div className="mt-3 grid gap-2 lg:grid-cols-2">
            ...
            <ReprintFile
              ...
              label="Лист комплектации"
              opening={state.openingKind === "details"}
              disabled={state.openingKind !== null}
              onOpen={() => onOpen("details")}
            />
          </div>
          {state.openError && <p className="mt-3 text-sm font-medium text-red-700" role="alert">Не удалось открыть PDF. Файл остаётся сохранённым.</p>}
        </div>
      </div>
    </section>
  );
...
function ReprintFile({ fileName, label, opening, disabled, onOpen }: {
  fileName: string;
  label: string;
  opening: boolean;
  disabled: boolean;
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${success ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800"}`}><span className={`size-1.5 rounded-full ${success ? "bg-emerald-500" : "bg-red-500"}`} />{success ? "Успешно" : "Ошибка"}</span>;
}
...
function HistoryLoading() {
  return <section className="grid gap-3 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-5 shadow-[var(--shadow-panel)]" aria-label="Загрузка истории печати">{[0, 1, 2, 3].map((row) => <span className="h-16 animate-pulse rounded-xl bg-[var(--surface-muted)]" key={row} />)}</section>;
}

function HistoryError({ onRetry }: { onRetry: () => void }) {
  return <section className="grid min-h-64 place-items-center rounded-2xl border border-red-200 bg-red-50 p-8 text-center" role="alert"><div><AlertCircle className="mx-auto mb-3 text-red-600" aria-hidden="true" size={26} /><h3 className="font-semibold text-red-950">Не удалось загрузить историю</h3><p className="mt-2 text-sm text-red-800">Локальные задания не изменены. Повторите запрос.</p><button className="mt-4 rounded-xl bg-red-700 px-4 py-2.5 text-sm font-semibold text-white" type="button" onClick={onRetry}>Повторить</button></div></section>;
}

function HistoryEmpty({ filtered }: { filtered: boolean }) {
  return <section className="grid min-h-64 place-items-center rounded-2xl border border-dashed border-[var(--border-strong)] bg-[var(--surface-elevated)] p-8 text-center"><div><PackageOpen className="mx-auto mb-3 text-[var(--text-muted)]" aria-hidden="true" size={26} /><h3 className="font-semibold">{filtered ? "Задания не найдены" : "История печати пока пуста"}</h3><p className="mt-2 text-sm text-[var(--text-secondary)]">{filtered ? "Измените запрос или статус." : "Успешные и неудачные PDF-задания появятся здесь автоматически."}</p></div></section>;
}
