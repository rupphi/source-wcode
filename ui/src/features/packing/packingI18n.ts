...
const ru = {
  ...
  pagination: { label: "Пагинация очереди упаковки", previous: "Предыдущая страница очереди", next: "Следующая страница очереди" },
  ...
  supplies: {
    ...
    modeUnknown: "Не указана", prepareDelivery: "Проверить передачу {name}", deliver: "Передать", open: "Открыть",
  },
...
const en: PackingCopy = {
  ...
  pagination: { label: "Packing queue pagination", previous: "Previous queue page", next: "Next queue page" },
  ...
  supplies: { columns: { supply: "Supply", mode: "Mode", created: "Created", orders: "Orders", action: "Action" }, modeUnknown: "Not specified", prepareDelivery: "Check delivery of {name}", deliver: "Deliver", open: "Open" },
...
const vi: PackingCopy = {
  ...
  pagination: { label: "Phân trang hàng đợi đóng gói", previous: "Trang hàng đợi trước", next: "Trang hàng đợi sau" },
  ...
  supplies: { columns: { supply: "Lô giao hàng", mode: "Mô hình", created: "Đã tạo", orders: "Đơn hàng", action: "Thao tác" }, modeUnknown: "Chưa xác định", prepareDelivery: "Kiểm tra giao lô {name}", deliver: "Giao hàng", open: "Mở" },
...
const zh: PackingCopy = {
  ...
  pagination: { label: "打包队列分页", previous: "上一页队列", next: "下一页队列" },
  ...
  supplies: { columns: { supply: "供货", mode: "模式", created: "创建时间", orders: "订单", action: "操作" }, modeUnknown: "未指定", prepareDelivery: "检查供货 {name} 的交付", deliver: "交付", open: "打开" },
  loading: "正在加载打包队列",
