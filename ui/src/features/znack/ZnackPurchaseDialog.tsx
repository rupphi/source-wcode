...
import { defaultPurchaseDialogCopy, type PurchaseDialogCopy } from "./ZnackPurchaseCopy";
...
export function ZnackPurchaseDialog({
  ...
  onClose,
  onStarted,
  copy = defaultPurchaseDialogCopy,
}: {
  shopId: number;
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-labelledby="purchase-dialog-title">
      <div ref={dialogRef} className="w-full max-w-lg rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h4 id="purchase-dialog-title" className="text-lg font-semibold">{preview ? copy.confirmTitle : copy.prepareTitle}</h4>
            <p className="mt-1 text-sm text-[var(--text-muted)]">{product.productName || copy.unnamedProduct}</p>
          </div>
          <button ref={initialFocusRef} className="icon-button" type="button" aria-label={copy.close} disabled={busy} onClick={onClose}><X aria-hidden="true" size={18} /></button>
        </div>
        <div className="mt-4 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-muted)] p-4">
          ...
          {preview ? (
            <>
              <div className="mt-3 flex items-center justify-between gap-3"><span className="text-xs text-[var(--text-muted)]">{copy.quantity}</span><strong className="text-sm">{preview.quantity}</strong></div>
              {preview.autoIntroduction ? <p className="mt-4 flex items-start gap-2 rounded-lg bg-[var(--warning-soft)] p-3 text-xs leading-5 text-[var(--text-secondary)]"><ShieldCheck aria-hidden="true" className="mt-0.5 shrink-0 text-[var(--warning)]" size={14} /><span><strong className="block text-[var(--text-primary)]">{copy.autoIntroductionTitle}</strong>{copy.autoIntroductionDescription}</span></p> : <p className="mt-4 text-xs leading-5 text-[var(--text-muted)]">{copy.localOnly}</p>}
            </>
          ) : (
        ...
        {!canPurchase ? <p className="mt-3 text-xs leading-5 text-[var(--warning)]">{copy.certificateRequired}</p> : null}
        <p className="mt-4 text-xs leading-5 text-[var(--text-muted)]">{copy.paidWarning}</p>
        <div className="mt-5 flex justify-end gap-2">
          <button className="secondary-button" type="button" disabled={busy} onClick={onClose}>{copy.cancel}</button>
          {preview ? (
            <button className="primary-button" type="button" aria-label={copy.confirmLabel} disabled={!canPurchase || busy} onClick={() => void confirm()}>{busy ? <RefreshCw aria-hidden="true" className="animate-spin" size={16} /> : <ShoppingCart aria-hidden="true" size={16} />}{busy ? copy.starting : copy.confirm}</button>
          ) : (
            <button className="primary-button" type="button" aria-label={copy.prepareLabel} disabled={!canPurchase || busy} onClick={() => void prepare()}>{busy ? <RefreshCw aria-hidden="true" className="animate-spin" size={16} /> : <ShieldCheck aria-hidden="true" size={16} />}{busy ? copy.checking : copy.prepare}</button>
          )}
        </div>
      </div>
    </div>
