...
public final class ZnackRepository {
    ...
    public long createPipeline(String gtin,int quantity,String requestKey){String key=requestKey==null||requestKey.isBlank()?java.util.UUID.randomUUID().toString():requestKey;String now=Instant.now().toString();try(Connection c=Database.getConnection();PreparedStatement ps=c.prepareStatement("INSERT INTO znack_purchase_pipelines(shop_id,gtin,quantity,request_key,stage,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",Statement.RETURN_GENERATED_KEYS)){ps.setInt(1,shop.shopId());ps.setString(2,GtinNormalizer.normalize(gtin));ps.setInt(3,quantity);ps.setString(4,key);ps.setString(5,PurchaseStage.VALIDATING.name());ps.setString(6,now);ps.setString(7,now);ps.executeUpdate();try(ResultSet r=ps.getGeneratedKeys()){r.next();return r.getLong(1);}}catch(SQLException e){throw new RuntimeException(e);}}
