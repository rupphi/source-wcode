...
public final class ZnackRepository {
    ...
    public void updateProductLabelMetadata(String gtin,String gender,String size){
        execute("UPDATE znack_products SET gender=?,size=? WHERE shop_id=? AND gtin=?",ps->{
            ps.setString(1,blankToNull(gender));
            ps.setString(2,blankToNull(size));
            ps.setInt(3,shop.shopId());
            ps.setString(4,GtinNormalizer.normalize(gtin));
        });
    }
    ...
    public void updateOrder(long id,String external,String remote,OrderStatus status,String error){execute("UPDATE kiz_orders SET external_order_id=COALESCE(?,external_order_id),remote_status=?,local_status=?,error_message=?,updated_at=? WHERE shop_id=? AND id=?",ps->{ps.setString(1,external);ps.setString(2,remote);ps.setString(3,status.name());ps.setString(4,ZnackSanitizer.report(error));ps.setString(5,Instant.now().toString());ps.setInt(6,shop.shopId());ps.setLong(7,id);});}
    ...
    public void updatePipeline(long id,Long orderId,PurchaseStage stage,String error){execute("UPDATE znack_purchase_pipelines SET order_id=COALESCE(?,order_id),stage=?,error_message=?,updated_at=? WHERE shop_id=? AND id=?",ps->{if(orderId==null)ps.setNull(1,Types.BIGINT);else ps.setLong(1,orderId);ps.setString(2,stage.name());ps.setString(3,ZnackSanitizer.report(error));ps.setString(4,Instant.now().toString());ps.setInt(5,shop.shopId());ps.setLong(6,id);});}
