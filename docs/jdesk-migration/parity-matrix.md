...
| Packaging | Maven `release.yml`, jpackage | release workflow now gates Maven/Gradle/frontend/audit, builds the jDesk app-image, uses a stable upgrade UUID, signs/verifies launcher + EXE + MSI, creates and independently checks the Ed25519 MSI manifest, publishes MSI/EXE/portable/manifest through a draft and fails closed on missing secrets; first provisioned Windows release and SBOM evidence remain pending | Foundation (signed workflow) |
