...
| Language/theme | `I18nService`, `ThemeService` | bounded shared `app_language`/`app_theme` persistence, RU/EN/VI/ZH shell/settings/license plus complete FBS supplies, FBS packing, FBO label-printing and GTIN/KIZ mapping journeys, Znack settings/products, and dark/light/system runtime behavior are covered by Java/React tests plus browser/native restart evidence; Znack operations and other feature-specific Russian copy remain pending | Foundation (shared + feature slices) |
