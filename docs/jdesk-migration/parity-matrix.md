...
| Packaging | Maven `release.yml`, jpackage | release workflow gates Maven/Gradle/frontend/audit, builds the jDesk app-image through a descriptor-safe jdeps analysis, embeds main + offline recovery launchers, uses a stable upgrade UUID, signs/verifies both launchers + EXE + MSI, regenerates checksums/CycloneDX/SPDX after signing, checks the Ed25519 MSI manifest and publishes through a draft; first provisioned Windows release remains pending | Foundation (signed workflow) |
