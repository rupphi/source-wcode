...
| Supply list | `SupplyListController` | bounded local list/search/status now appends validated pages through observer or keyboard fallback, deduplicates stable IDs, hides stale filters immediately and preserves loaded rows on later-page error; React and isolated browser evidence cover 320–1440px without horizontal overflow, while live native list/detail-state restore remains covered by the existing page contract | Foundation |
| Supply detail/orders | `SupplyDetailController` | local detail/search/natural sort now appends bounded order pages with observer/fallback and responsive 320–1440px table evidence; opaque image assets and live WB refresh remain covered, and local GTIN inventory also appends bounded batches while license/certificate-gated purchase confirmation stays outside scroll behavior | Foundation |
